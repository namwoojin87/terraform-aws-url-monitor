...
The bootstrap customer-managed key, alias, key policy and scoped deployment permissions were applied and verified on 2026-09-07 at 09:04:34 UTC. The runtime publisher policies and encrypted SNS topic still await deployment; key creation does not verify either encrypted delivery path. See the [bootstrap apply record](bootstrap-hardening-apply-2026-09-07.md).

Deployment has two separately approved stages. Stage 1 is complete for the recorded plan; do not reapply it. Any future bootstrap change requires its own fresh plan and approval:
...
The Lambda publisher retains `sns:Publish` through its project IAM policy. The pending runtime change limits its KMS use to `kms:GenerateDataKey*` and `kms:Decrypt` on the exact alert key, through SNS in Seoul, with the exact alert-topic encryption context.

The pending runtime topic policy grants CloudWatch `sns:Publish` only for the exact account and `url-monitor-lambda-errors` alarm. The deployed key policy grants only the required cryptographic actions to the CloudWatch service with the same exact account, alarm, and topic encryption context. Combining those documented CloudWatch source restrictions with the SNS encryption context is a policy-design inference, not proof of live delivery.
