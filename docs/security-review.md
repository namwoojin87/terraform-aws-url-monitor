...
| --- | --- | --- | --- |
| Repository Terraform after local bootstrap remediation, including bootstrap, live root, and local module | 1 | 1 | 30 resources; 154 passed checks; 18 failed; 0 skipped; 0 parsing errors |
...
Every row below is **proposed, not approved, and not applied to the gate**. An existing cost or operating constraint explains a finding; it does not silently accept its residual risk. All runtime resources below are in `modules/url-monitor/main.tf`; addresses include the `module.url_monitor` prefix reported by the full-root scan. The two table rows each cover exactly the two listed addresses, not every DynamoDB table.

| Check | Exact resource address | Existing design, residual risk, and review trigger |
| --- | --- | --- |
| `CKV_AWS_26` | `module.url_monitor.aws_sns_topic.alerts` | Topic encryption is not configured. Messages describe monitor transitions and must not contain credentials. Accepting unencrypted SNS storage requires an owner decision. Enabling encryption must preserve both Lambda and CloudWatch publishers; see the key-policy note below. Revisit before sending sensitive payloads. |
| `CKV_AWS_28` | `module.url_monitor.aws_dynamodb_table.state`; `module.url_monitor.aws_dynamodb_table.history` | State and check history use seven-day TTL and no point-in-time recovery. Lost records cannot be restored from PITR; state loss may affect notification transitions. Proposed only while this is disposable monitoring data. Revisit if records become an audit or recovery requirement. |
...
| `CKV_AWS_116` | `module.url_monitor.aws_lambda_function.checker` | No Lambda DLQ is configured. A failed invocation may leave an unrecorded check after retries, and there is no queue for replay. The existing error alarm does not recover the lost event. Revisit if missing checks must be recoverable. |
...
| `CKV_AWS_50` | `module.url_monitor.aws_lambda_function.checker` | Existing service metrics and short-lived logs are used without X-Ray. Cross-service trace evidence is unavailable. Revisit if troubleshooting requires distributed traces. |
...
SNS is a separate case: encryption is not configured on this topic. AWS states that [CloudWatch alarm delivery to encrypted SNS requires a customer-managed key](https://repost.aws/knowledge-center/cloudwatch-configure-alarm-sns), with [appropriate publisher key permissions](https://docs.aws.amazon.com/sns/latest/dg/sns-key-management.html). Merely adding `alias/aws/sns` to satisfy a static check can break the alarm path. Any change needs a key/cost decision and verification of both publishers.
